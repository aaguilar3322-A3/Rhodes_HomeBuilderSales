...
region_sales AS(
    SELECT
    DENSE_RANK() OVER (PARTITION BY Year ORDER BY SALES_TARGET_PCT DESC) SALES_TARGET_Rank
    , sc.*
    FROM(
        select
        r.Year,
       ...
       from {{ ref('v_regional_manager_sales') }} r
        GROUP BY year, region, regional_manager, rm_sales_target_units, rm_margin_target_pct
    ) sc
)
