...
WITH 
sales_consultant_closed_sales_yearly AS(
    SELECT
    DENSE_RANK() OVER (PARTITION BY sc.Year ORDER BY sc.year, Total_Closed DESC) Total_Closing_Rank
    , sc.*
    FROM(
        select
        r.year
        ...
        GROUP BY r.year, sales_consultant
    ) sc
)

select * from sales_consultant_closed_sales_yearly